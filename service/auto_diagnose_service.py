...
def run(progress_callback=None):
    ...
    if unique_ips:
        ...
        try:
            ...
            sorted_results = sorted(test_results.items(),
                                    key=lambda x: x[1]['latency'] if x[1]['success'] else float('inf'))
            if sorted_results:
    ...
    for retry in range(max_retries):
        try:
            ...
            update_progress(5, f"验证异常，正在重试 ({retry + 1}/{max_retries}): {e}...")
            time.sleep(2)
    ...
    if verify_status != "good":
        ...
        for backup_ip in backup_ips:
            if backup_ip != best_ip:
                ...
                update_progress(6, f"备选IP修复结果: {'成功' if backup_repair_result.get('success', False) else '失败'}")
                time.sleep(2)
